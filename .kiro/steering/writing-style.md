...
- **Reference by heading name, not by number.** When pointing at another section, sub-part, chunk, or note, use the heading text — strip any leading number. Numbers shift constantly: chunks reorder, sub-parts split, headings get auto-renumbered by tooling, new content gets inserted. Heading names are the only stable handle. The rule is absolute:
  - **Strip the number when referencing a numbered heading.** A heading `## 1.1. This is the heading` is referenced as *"This is the heading"* — never as *"§1.1"*, *"section 1.1"*, *"1.1. This is the heading"*, or *"chunk 1"*.
  - **No numeric cross-references anywhere** — not across notes, not within a note, not in asides, not in tables, not in code-comment annotations.
  - **Heading numbers themselves are tooling-managed.** They're file-local organization, not part of the canonical heading identity. Don't bake them into prose by saying *"as covered in 1.5.2"* — the tool that adds/removes those numbers will desync your text from the headings.
  - **Forward references to other courses go by course name** — e.g. *"covered in the DOM fundamentals course"*, not *"covered in course #9"*.
...
At every point, the reader understands using only what came before.
...
### Asides in blockquotes

Tangential content that's useful but sits outside the main derivation — terminology sidebars, meta-notes, historical context, formal-layer remarks — goes in a `> **Aside —** …` blockquote rather than its own heading. The note should read top-to-bottom without the asides and still land. Guardrails:
...
- **One role per blockquote prefix.** Reserved prefixes:
  - `> ⚠️` — content the reader shouldn't take at face value. Two uses, both signal "this content is suspect":
    - **Critical-lens flag on source content** — *"⚠️ The original source claims X, but…"*
    - **Self-correction marker on teaching drafts** — *"⚠️ Corrected during teaching — initial framing said X; actual mechanism is Y."*
  - `> **Aside —**` — tangents (terminology sidebars, meta-notes, historical context, formal-layer remarks).
  - `> 🔖 Later:` — explicit rabbit hole flagged for future exploration.

  Don't mix prefixes across roles.
...
- **One clause per sentence when the technical content is dense.** Split on *and*, *but*, *which*, *because* if the sentence already carries a non-trivial mechanism. Two short sentences read faster than one accurate-but-stuffed one. The denser the mechanism, the shorter the sentences. Never compensate for complexity by packing more into one sentence — sequence the pieces instead of nesting them.
