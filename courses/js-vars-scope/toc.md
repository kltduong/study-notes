...
- [ ] **TDZ** — design rationale, what "temporal" means, identifying TDZ in practice, relationship to the creation phase model
      🔧 Refinements (in-progress):
        - Said "inside the block, there is a new EC" — block entry creates a new Block ER and moves `LexicalEnvironment` to it; no new EC. ECs are only pushed on function calls (plus script/eval). Covered in `creation-execution.md`; the refinement is re-surfacing the engine-vs-ER distinction under pressure.
        - (Teaching-side, not learner) Draft initially used a weak example for "runtime vs lexical" — two reads at different source positions — undermining the "same position, different time" point. Learner caught it; replaced with the teaser example (one fixed read called twice). Worth flagging because it was a reasoning slip on the teacher's side that the learner's critical reading surfaced.
