...
- [x] **Quiz: scope & declarations** — covers chunks 6–9
      ...
      🔧 Refinements:
        ...
        - Q3 vocabulary: said `Object.freeze` makes "field mutable to false." Precise: each property has a **descriptor** `{ value, writable, enumerable, configurable }`; freeze sets `[[Writable]]` and `[[Configurable]]` to `false`. Recurring **descriptor-vs-value** pattern from chunk 9 / cross-cutting weaknesses.
        - Q3 L2 location: said the `config` binding lives in "global ER." Precise: in script mode, top-level `const` goes into the Global ER's `[[DeclarativeRecord]]` (the hidden table), not the `[[ObjectRecord]]`. In a module, the Module ER. The composite Global ER framing from chunk 8 should route this on sight.
        ...
        - Q4 (b): said "newEC.[[OuterEnv]]" — the **third surface** of the `[[Environment]]`-slot conflation (after chunk 6's `[[Environment]]`-vs-`[[OuterEnv]]` and chunk 7's definition-vs-call-time slip). `[[OuterEnv]]` is on the **Environment Record**, not the EC. The EC carries `LexicalEnvironment` / `VariableEnvironment` pointers to ERs; the ER carries `[[OuterEnv]]` for the chain. Named explicitly with a three-row table this pass — promote to cross-cutting weakness at end-of-course.
- [ ] **Final test**
