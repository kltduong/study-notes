...
```css
a[href^="https"]         { /* external, secure */ }
a[href$=".pdf"]::after   { content: " 📄"; }
img[alt=""]              { /* empty alt — decorative */ }
img:not([alt])           { outline: 3px solid red; /* missing alt = bug */ }
[class*="icon-"]         { /* any icon-* class */ }
...
```css
p:nth-child(2) { /* "First" — child #2 AND a p */ }
p:nth-child(1) { /* nothing — child #1 is an h2 */ }
```
...
```css
p:nth-of-type(1) { /* "First" — 1st p, ignoring the h2 */ }
...
tr:nth-child(3n+1)  { border-top: 2px solid; }
li:nth-child(-n+3)  { font-weight: bold; /* first three */ }
```
...
```css
li:nth-child(2 of .featured) { /* 2nd li with class .featured */ }
```

**Not** the same as `.featured:nth-child(2)`, which requires the 2nd child overall to *also* be `.featured`.
...
```css
.tag:only-child    { margin: 0; /* no siblings, no gap */ }
section:only-of-type { /* the only <section> in its parent */ }
```

## See also

- [selectors-basics.md](./selectors-basics.md) — the pseudo-class category these live in.
- [specificity-and-cascade.md](./specificity-and-cascade.md) — these pseudo-classes count toward slot `b`.
