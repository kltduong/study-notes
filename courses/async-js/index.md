...
Start with [what-is-async](what-is-async.md) for the foundational "why" — why async exists, the three-part model (engine / runtime / event loop). Then [event-loop-basics](event-loop-basics.md) zooms into the mechanics of the call stack and task queue. [callbacks](callbacks.md) and [callback-problems](callback-problems.md) cover the original async pattern and why it needed replacing. [testing-callbacks](testing-callbacks.md) covers how to test async callback code with Mocha/Chai. From there, [promise-fundamentals](promise-fundamentals.md) introduces the language-level solution to callback problems, [working-with-promises](working-with-promises.md) covers promisification, chaining, and branching, [fetch-rejections](fetch-rejections.md) covers the Fetch API, `.catch()`/`.finally()`, and rejection handling patterns, [parallel-combinators](parallel-combinators.md) covers `Promise.all` and `Promise.allSettled` for running promises in parallel, [race-any](race-any.md) completes the combinator picture with `Promise.race` and `Promise.any`, [testing-promises](testing-promises.md) covers testing promise-based code with Mocha/Chai, and [async-await](async-await.md) covers the syntactic layer on top of promises — suspension mechanics, error handling, and sequential vs parallel execution. [microtasks](microtasks.md) closes the loop by upgrading the event-loop model with task priorities, explaining why promise chains feel atomic and why long-task yielding must go through the task queue. [animations](animations.md) covers debugging async in DevTools, why `setInterval` breaks animation, and how `requestAnimationFrame` syncs to the render step. Finally, [event-loop-wrapup](event-loop-wrapup.md) consolidates the complete event loop model — the six-step cycle, scheduling hierarchy, and the rules that predict any execution order.
...
- [race-any.md](race-any.md) — `Promise.race` (first to settle wins) and `Promise.any` (first fulfillment wins, `AggregateError` on total failure), timeout pattern, empty-input behaviors, from-scratch implementations.
- [testing-promises.md](testing-promises.md) — Testing promises in Mocha: return the promise, forget-return trap, rejection testing with fulfillment guards, don't mix `done` and promises.
- [async-await.md](async-await.md) — Async function execution model, `await` suspension, try/catch for async errors, sequential vs parallel patterns, `return await` vs `return`, top-level await.
- [microtasks.md](microtasks.md) — Two-queue event loop model, promise state machine + `.then()` scheduling, microtasks vs tasks, long-task yielding (must use tasks not microtasks), event handler atomicity, `queueMicrotask`.
- [animations.md](animations.md) — Debugging async in DevTools (async stack traces, Performance tab), why `setInterval` breaks animation, `requestAnimationFrame` and the render step, frame-rate-independent motion.
- [event-loop-wrapup.md](event-loop-wrapup.md) — The complete six-step event loop model, three-layer architecture, scheduling hierarchy, and the five rules that predict any execution order.
