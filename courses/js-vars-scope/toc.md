...
**Pacing notes:** *Execution context internals* and *Creation & execution phases* are the core — spend time there. *Variable lifecycle*, *`var` quirks & historical patterns*, and *`const` & immutability* are lighter (review + new angle). Probe descriptor-vs-value confusion and engine-vs-runtime conflation from cross-cutting weaknesses where relevant.
...
- [x] **Quiz: execution model** — covers *Variable lifecycle* through *TDZ*
...
- [x] **Lexical scoping & shadowing** — scope chain resolution, nested scopes, shadowing rules, closures (ER survival via `[[OuterEnv]]`), lexical vs dynamic scoping (with Bash contrast)
      ...
      🔧 Refinements:
        - Q1: said "a.[[Environment]] points to build Function ER when calling a()" — slot is actually set at function-object *allocation* (during the surrounding call's execution), not at the inner function's call time. Same definition-time-vs-call-time compression as the *Scope model* chunk's `[[Environment]]` refinement. **Recurring pattern.**
...
- [x] **`const` & immutability** — reassignment vs mutation, `Object.freeze`, shallow vs deep, use cases
      📊 solid — axiom (binding vs value) landed cleanly; spec mechanism (one flag in `SetMutableBinding`, `Initialize` ≠ `Set`) derived without prompting; Q1–Q3 all mechanism-correct on first pass; binding-vs-value applied across closure boundary in Q3
      🔧 Refinements:
        - End-of-chunk Q1: didn't surface that L4's strict-mode `TypeError` halts execution → L5/L6/L8/L9 unreachable in the as-written script. Per-line mechanism was right but missed the script-level sequencing consequence. Same pattern as the sloppy/strict difference: silent vs early-death, observable through *what runs after the failure*.
        - End-of-chunk Q1 (under "assume L4 skipped"): missed the L5 `push` effect when tracing L9. Per-line answers were right; chained trace dropped one of the mutations. Watch for trace-composition errors when multiple mutations stack on the same object.
        - Q2 vocabulary: said `const user` "frozen the binding" — language slip mixing the two mechanisms. Precise: `const` sets the binding's `mutable` flag to `false`. Reserve "frozen" for `Object.freeze`. Mixing the words obscures that they're separate mechanisms.
        - Q2 (a)/(c): described frozen-property mechanism as "the field property to modify is set to false." Precise vocabulary: each property has a **descriptor** (a `{ value, writable, enumerable, configurable }` record) whose `[[Writable]]` field is set to `false`. Recurring **descriptor-vs-value** pattern from cross-cutting weaknesses — surfaced again here.
- [x] **Quiz: scope & declarations** — covers *Scope model* through *`const` & immutability*
      📊 solid — all four questions reached the correct mechanism, several after one narrowing nudge each. Recurring patterns surfaced and named explicitly: descriptor-vs-value, and the `[[Environment]]`-slot-on-function-object vs `[[OuterEnv]]`-on-ER vs `LexicalEnvironment`-on-EC three-way distinction.
      🔧 Refinements:
        - Q1 (b): said "1, 2, 3" for the `let` variant — off-by-one on values (counted iteration ordinal rather than `i`'s value at push time). Mechanism reasoning ("per-iteration binding") was correct. Self-corrected immediately on a narrow nudge. Also a tightening: `for (let i = ...)` uses a dedicated **iteration environment** that copies the previous iteration's value forward into the fresh binding — not just "a Block ER per iteration."
        - Q2 IIFE: initial answer named the two **syntactic** pieces (wrapping parens, trailing parens) rather than the two **semantic** channels. After narrowing, recovered: (1) function call pushes a Function ER → `var` isolation, (2) immediate invocation runs the body in place. Wrapping parens are a parser-position fix (FunctionDeclaration → FunctionExpression), not a channel.
        - Q3 vocabulary: said `Object.freeze` makes "field mutable to false." Precise: each property has a **descriptor** `{ value, writable, enumerable, configurable }`; freeze sets `[[Writable]]` and `[[Configurable]]` to `false`. Recurring **descriptor-vs-value** pattern from the *`const` & immutability* chunk / cross-cutting weaknesses.
        - Q3 L2 location: said the `config` binding lives in "global ER." Precise: in script mode, top-level `const` goes into the Global ER's `[[DeclarativeRecord]]` (the hidden table), not the `[[ObjectRecord]]`. In a module, the Module ER. The composite Global ER framing from the *`var` quirks & historical patterns* chunk should route this on sight.
        - Q4 (a): said `inner.[[Environment]]` is set "after L5." Imprecise — it's set during the **creation phase** of `outer`'s call (L8), before any of L2–L6 runs. Function-declaration hoisting allocates the function object and sets its `[[Environment]]` slot during creation phase. Recovered with the `typeof inner` at L1.5 probe.
        - Q4 (b): said "newEC.[[OuterEnv]]" — the **third surface** of the `[[Environment]]`-slot conflation (after the *Scope model* chunk's `[[Environment]]`-vs-`[[OuterEnv]]` and the *Lexical scoping & shadowing* chunk's definition-vs-call-time slip). `[[OuterEnv]]` is on the **Environment Record**, not the EC. The EC carries `LexicalEnvironment` / `VariableEnvironment` pointers to ERs; the ER carries `[[OuterEnv]]` for the chain. Named explicitly with a three-row table this pass — promote to cross-cutting weakness at end-of-course.
- [ ] **Final test**
