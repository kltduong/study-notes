...
**TL;DR:** A class body is just sugar over the same `[[Call]]`/`[[Construct]]` machinery — no class-specific `this` rule. Method shorthand creates non-constructable functions on the prototype (one shared object, `[[IsConstructor]] = false`, `[[HomeObject]]` set). Class fields are per-instance properties whose initializers run during `[[Construct]]` (in source order, before the body) with `this` = the fresh instance. Arrow fields exploit this to lock `this` at construction time, structurally surviving extraction at the cost of one function object per instance. Derived constructors don't create the object themselves — `super()` does, by calling up the chain to the base constructor (with `newTarget` propagated so the prototype is right). Until `super()` returns, `[[ThisValue]]` is **uninitialized** — any `this` access throws ReferenceError (the `this`-TDZ). Field ordering: base fields → base body → `super()` returns → derived fields → derived body. Omit the constructor and the engine synthesizes one (`constructor() {}` for base, `constructor(...args) { super(...args); }` for derived — `...args` preserves arity).

> **Aside —** "Class method" in JS ≠ Python's `@classmethod`. A JS class method is an *instance* method (Python's `def bark(self):`). JS `static` is closer to Python's `@classmethod` — called on the class, `this` = the constructor. Key difference: Python's `@classmethod` descriptor *guarantees* `cls` regardless of call form; JS `static` still follows Reference-base and can lose `this` on extraction.

## 1.1. Method shorthand — non-constructable, same `this` rule

A method defined via **shorthand syntax** (inside a `class` body or object literal) is an ordinary function object — same `[[Call]]`, same `this`-determination pipeline. Two structural differences vs function declarations/expressions:

| Property | Function declaration/expression | Method shorthand |
|---|---|---|
| `[[IsConstructor]]` | `true` | `false` |
| `[[HomeObject]]` | not set | set to the class's `.prototype` (or the literal) |
...
const dog = {
  bark: function() { return "woof"; } // function expression as property — constructable (legacy)
};
const dog2 = {
  bark() { return "woof"; }           // method shorthand in literal — non-constructable
};
// dog2.bark and Dog.prototype.bark have identical structural properties —
// the shorthand syntax does the work, not the class keyword.
```

`[[IsConstructor]] = false` is enforced at creation time based on syntactic form. Same gate arrows hit, but for a different reason: arrows lack `this` machinery entirely; methods *have* `this` machinery but are semantically "not standalone constructors."

`[[HomeObject]]` enables `super.method()` calls — a static link to the prototype object the method was defined on. Not relevant to `this` determination, but it's the second structural difference (it feeds into the inheritance topic).

### 1.1.1. `this` follows the same Reference-base rule
...
No class-specific `this` rule. L8 works because `d.bark()` is a member-expression call → Reference base = `d`. L10 fails because `fn()` is an identifier call → Reference base = ER → `undefined` in strict.

> **Aside —** Class bodies are always strict, regardless of surrounding code. The sloppy-mode `globalThis` fallback never applies inside a class method — `this` is `undefined` on plain calls, never the global object.

### 1.1.2. Why non-constructable matters

```js
new d.bark();                                         // TypeError: d.bark is not a constructor
```

The engine checks `[[IsConstructor]]` before running `[[Construct]]`. Method shorthand → `false` → immediate TypeError. **Design-time guarantee:** methods can't accidentally be used as constructors. No `.prototype` property is created either (saves memory, signals intent).

## 1.2. Method extraction and `this`-loss
...
bus.emit();                                           // L14 — TypeError when log runs
```
...
2. **L4 — storage.** `this.handlers.push(fn)` stores that bare function object into the array. Still just a value — no Reference wrapper.
3. **L5 — the call that loses `this`.** `.forEach(fn => fn())`. The call expression `fn()` resolves `fn` via `ResolveBinding` → Reference with base = the arrow's Function ER (an Environment Record, not an object). Call operator: `IsPropertyReference`? No (base is ER) → `thisValue = undefined`.
...
The function object is identical — same `.name`, same body, same `[[HomeObject]]`. The difference is purely in what the **call-site expression** looks like.

### 1.2.2. Common extraction sites
...
### 1.2.3. Why classes make this worse

Two factors compared to pre-class code:
...
### 1.2.4. The fundamental tension

Classes encourage an OOP style where behavior lives on instances (`this.method()`). But JavaScript's `this` is **per-call, not per-object** — determined by the call expression, not by where the function "belongs." Any API boundary that accepts a function strips the object context.

This tension motivates the solutions that follow (arrow fields below; `bind`-in-constructor as the older alternative).

### 1.2.5. The flip side: capabilities per-call `this` enables

The same mechanic that breaks extraction is what enables JS to do things Java can't and Python only partially supports. The flexibility is real — but in modern application code, almost every capability has a clearer non-`this` replacement, so actual *use* is narrow.
...
**The pattern.** Per-call `this` is a sharp generic-dispatch tool. Real practical use today is narrow — it lives in **library/framework internals**, **polyfills/shims**, and **reading/maintaining old code**. For application code, modern JS has converged on:
...
## 1.3. Field initializers — when they run, what `this` they see

### 1.3.1. What a field is, mechanically

An instance field declares a property that the engine assigns **per instance** during construction. (Static fields are per-class, run once at class evaluation time.) The syntax:
...
class Foo {                                           // L1
  ...
  constructor() {                                    // L5
    ...
    this.count = 99;                                 // L7 — body overwrites
  }                                                  // L8
}                                                    // L9
```

is sugar for: "during `[[Construct]]`, after `this` exists, run `this.count = 0` (L2) and `this.#secret = "hidden"` (L3), then run the constructor body (L5–L8)." Instance fields are not stored on the prototype — each instance gets its own copy. **Structural opposite of methods** (which live on the prototype, shared).

### 1.3.2. When initializers run

Each instance field has its own **initializer** — the right-hand side expression. Internally, the engine wraps each initializer in a separate function called with `this` = the fresh instance. Fields without an initializer (`name;`) default to `undefined`.

In a **base class** (no `extends`), initializers run at the start of the constructor, immediately after `this` is bound:
...
L6 sees `0` because the field initializer (step 3) ran before the constructor body (step 4). The constructor can read and overwrite fields that already exist on `this`.
...
Field initializers run as part of the constructor's execution context. So `this` inside an initializer is the fresh instance:
...
L4 demonstrates that earlier fields are visible to later ones — they run in source order, each assigning to `this` before the next runs.

> **Aside —** Spec mechanics: each field is internally a function (the "initializer function") whose `[[HomeObject]]` is the class's `.prototype` (instance fields) or the class itself (static fields). The constructor calls each via a special `[[Call]]` that uses the fresh instance as `thisValue`. Inside an initializer, `this` follows the same `[[ThisValue]]`-slot mechanism as a method body — just invoked by the engine, not your code.
...
### 1.3.5. Tradeoff: per-instance vs prototype sharing

Arrow fields trade memory for safety:
...
- **Method (default).** Most class members. Sharing on the prototype is the right default — it's why prototypes exist. Use when callers will invoke as `obj.method()`.
...
### 1.3.6. What goes wrong if you reach for arrow fields by default
...
L8 trace: After `super()` runs, `Animal`'s field initializer assigns the arrow to `this.speak` — directly on the instance. `Dog.prototype.speak` exists too, but property lookup finds the **instance property first**. The subclass method is shadowed by the parent's instance field.
...
If you need `super`, use a method, not an arrow field.

> **Aside — function-expression fields are the worst of both worlds.** `log = function () {...}` (regular function as field initializer) gives you per-instance memory cost (one function per instance, like an arrow) but **no `this`-locking** (regular functions read `this` from the call site, like a method). There's almost no reason to write this — if you want per-instance + locked `this`, use an arrow field; if you don't need locking, use method shorthand on the prototype. Useful only as a teaching example to show that "field" and "arrow `this`-locking" are independent properties.

## 1.4. `super()` as `this`-provider in derived constructors

### 1.4.1. The structural asymmetry

In a base class, `[[Construct]]` creates the fresh object *before* the constructor body runs. The constructor's `[[ThisValue]]` is set immediately — fields and the body both see `this` from the start.

A derived class **cannot do this**. The fresh object must have the correct `[[Prototype]]` — which depends on the *most-derived* constructor's `.prototype`. But the engine enters the *base* constructor first (via `super()` calls up the chain). The object can't be created until the base constructor runs.
...
"TDZ" (Temporal Dead Zone) is borrowed from `let`/`const` — the slot exists but accessing it before initialization throws. Same concept for `this` in a derived constructor:

```js
"use strict";                                         // L1
class Base {                                          // L2
  constructor() {                                    // L3
    console.log("Base: this exists");                // L4
  }                                                  // L5
}                                                    // L6
...
L9 throws `ReferenceError: Must call super constructor in derived class before accessing 'this' or returning from derived constructor`. The `[[ThisValue]]` slot in the derived constructor's Function ER is **uninitialized** — the engine checks on every `this` reference and throws if it hasn't been set yet.

L10 (`super()`) is what initializes it.

### 1.4.3. What `super()` actually does

`super()` is not just "call the parent constructor." It's the mechanism that **provides `this`** to the derived constructor:

```
Derived constructor's [[Construct]] (via `new Derived()`):
...
**The rule:** Base fields → Base body → (super returns) → Derived fields → Derived body. Each class's fields run before its own constructor body, but the entire base construction completes before derived fields start.

A consequence worth highlighting: if both classes declare the same field name, the derived assignment **overwrites** the base assignment on the same object — there's only one heap object, and field assignments are property writes on it.

```js
class Shape { type = "shape"; constructor() { console.log(this.type); } }
class Circle extends Shape { type = "circle"; constructor() { super(); console.log(this.type); } }
new Circle();                                         // logs "shape" then "circle"
```

`Shape`'s constructor body sees `"shape"` because `Circle`'s field initializer hasn't run yet — same fresh object, just a later write on the same property.

### 1.4.5. What you can and cannot do before `super()`
...
### 1.4.6. The "must call super" rule

A derived constructor **must** call `super()` before it returns (unless it explicitly returns an object — the return-value override from [Constructor calls](new-construct.md)):
...
The implicit `return this` at L4 tries to read `[[ThisValue]]` — still UNINITIALIZED → ReferenceError. The only escape hatch: `return someObject;` (explicit non-`this` return), which bypasses the `this` slot entirely.

### 1.4.7. Default constructors — when you omit `constructor`
...
The derived default forwards every argument to the parent — that's why omitting the constructor still works when the parent expects arguments:
...
The synthesis is purely a parser-level convenience. By the time `[[Construct]]` runs, there's a real function in place — **same `[[ThisValue]] = UNINITIALIZED` start, same `super()` providing `this`, same field-initializer ordering.** Field initializers still run; the synthesized constructor just has nothing else in its body.
...
If neither applies, omit it.

### 1.4.8. Why this design? (motivation)
...
3. **The TDZ catches real bugs** — accessing `this` before the object is fully set up would read uninitialized properties. ReferenceError is better than silent `undefined`.

Python comparison: `__init__` receives `self` already created (by `__new__`) — no TDZ, `self` is always available. But Python also can't enforce "base init ran first" — you can forget `super().__init__()` and get a half-initialized object silently. JS chose strictness over convenience.

## 1.5. Worked synthesis

A single annotated example tracing every mechanism above through one construction. Each commented line marks which section it demonstrates.
...
### 1.5.1. Trace of L32 — the construction sequence

`new FastCounter("fast")` triggers a chain of `[[Construct]]` calls:

```
1. FastCounter.[[Construct]]("fast", newTarget=FastCounter):
   - [[ThisValue]] = UNINITIALIZED (this-TDZ)
   - Enter L24 body
...
3. Component.[[Construct]]("fast", newTarget=FastCounter):
   ...
   - Run Component's field initializers (source order):
...
5. Back in FastCounter:
   - BindThisValue: FastCounter's ER [[ThisValue]] = fresh object  ← same object, now in scope
   - Run FastCounter's field initializers:
     • L23: this.step = 10                ← OVERWRITES the 1 written at L18
   - Run FastCounter's body:
     • L29: this.tick()                   ← Reference base = this → tick runs with this=instance
...
### 1.5.2. Trace of L34–L37 — extraction outcomes diverge
...
L35: safe()
  ...
  - WORKS — arrow field structurally locks this
...
L36: const lost = fc.tick
  ...
  - GetValue extracts the function from FastCounter.prototype's chain
    (lookup walks ...→ Component.prototype → finds tick)
  - lost = the method's function object
...
L37: lost()
  ...
  - FAILS — method extraction loses this
```

### 1.5.3. What this consolidates
...
- **No class-specific `this` rule** — class methods follow the same Reference-base rule as any function. Class bodies are always strict, so `this = undefined` on plain calls (no `globalThis` fallback).
- **Method shorthand is non-constructable** — `[[IsConstructor]] = false`, `[[HomeObject]]` set to the prototype, no `.prototype` property. One function lives on the prototype, shared across all instances.
- **Class fields are per-instance** — each `new` runs the initializers against that fresh instance, in source order, with `this` = the instance, *before* the constructor body runs.
- **Arrow field = `this`-locking** — combines per-instance storage with lexical `this` capture; survives extraction at the cost of one function object per instance. Use when methods are passed as callbacks; otherwise prefer prototype methods.
- **`super()` provides `this`** — in a derived constructor, the fresh object is created by the base (with `newTarget` propagated for the right prototype). Until `super()` returns, `[[ThisValue]]` is uninitialized — any `this` access throws ReferenceError.
- **Field ordering** — base fields → base body → `super()` returns → derived fields → derived body. Same heap object throughout; same-named fields overwrite.
- **Default constructors** — base class synthesizes `constructor() {}`; derived class synthesizes `constructor(...args) { super(...args); }`. The `...args` spread preserves arity.
